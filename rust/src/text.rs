//! Frozen argparse output.
//!
//! GENERATED from the Python reference -- do not hand-edit. Regenerate with the
//! snippet in bench/README.md if the reference's parser ever changes.
//!
//! argparse renders these from the parser definition, which is frozen, so they
//! are constants here rather than something clap-like is asked to reproduce.
//! `bench/equivalence.sh cli verify` diffs them against the live reference.

/// `isONclust --help` / `isONclust` with no arguments, on stdout.
pub const HELP: &str = r#"usage: isONclust [-h] [--version] [--fastq FASTQ] [--flnc FLNC] [--ccs CCS]
                 [--t NR_CORES] [--d PRINT_OUTPUT] [--q QUALITY_THRESHOLD]
                 [--ont] [--isoseq] [--use_old_sorted_file] [--consensus]
                 [--abundance_ratio ABUNDANCE_RATIO]
                 [--rc_identity_threshold RC_IDENTITY_THRESHOLD] [--medaka]
                 [--k K] [--w W] [--min_shared MIN_SHARED]
...
options:
  ...
  --d PRINT_OUTPUT      For debugging, prints status of clustering and
                        minimizer database every p reads processed. (default:
                        10000)
  --q QUALITY_THRESHOLD
                        Filters reads with average phred quality value under
  ...
  --use_old_sorted_file
                        Using already existing sorted file if present in
  ...
  --consensus           After clustering, (1) run spoa on all clusters, (2)
                        ...
                        False)
  --abundance_ratio ABUNDANCE_RATIO
                        Threshold for --consensus algorithm. Consider only
                        clusters larger than a fraction of number of total
                        reads (default 0.1) (default: 0.1)
  --rc_identity_threshold RC_IDENTITY_THRESHOLD
                        Threshold for --consensus algorithm. Define a reverse
  ...
  --batch_type BATCH_TYPE
                        In parrallel mode, how to split the reads into chunks
...
/// `isONclust write_fastq --help`, on stdout.
pub const WRITE_FASTQ_HELP: &str = r#"usage: isONclust write_fastq [-h] [--clusters CLUSTERS] [--fastq FASTQ]
                             [--outfolder OUTFOLDER] [--N N]

options:
  -h, --help            show this help message and exit
  --clusters CLUSTERS   the file "final_clusters.csv created by isONclust."
...
/// The usage block argparse prints to stderr before any error message.
pub const USAGE: &str = r#"usage: isONclust [-h] [--version] [--fastq FASTQ] [--flnc FLNC] [--ccs CCS]
                 [--t NR_CORES] [--d PRINT_OUTPUT] [--q QUALITY_THRESHOLD]
                 [--ont] [--isoseq] [--use_old_sorted_file] [--consensus]
                 [--abundance_ratio ABUNDANCE_RATIO]
                 [--rc_identity_threshold RC_IDENTITY_THRESHOLD] [--medaka]
...
/// `isONclust --version`, on stdout.
pub const VERSION: &str = "isONclust 0.0.6.1\n";
